...
export default function InputView() {
    ...
    // Transform data untuk Autocomplete
    ...
    return (
        <Box component="form" noValidate autoComplete="off" sx={{ pb: 4 }}>
            ...
            <Paper elevation={0} sx={{
                ...
                boxShadow: '0 2px 16px rgba(227, 30, 36, 0.04)'
            }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                    ...
                    <Box>
                        <Typography variant="body1" fontWeight="bold" color="primary" sx={{ fontSize: '0.95rem', lineHeight: 1.2 }}>
                            Step 1: Informasi Dasar
                        </Typography>
                ...
                <Stack spacing={2}>
                    ...
                    {/* ? AUTOCOMPLETE NAMA (COMBO BOX) */}
                    <Autocomplete
                        options={namaOptions}
                        value={nama}
                        onChange={(e, newValue) => setNama(newValue)}
                        ...
                        renderOption={(props, option) => (
                            <Box component="li" {...props} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 1 }}>
                                <Avatar sx={{ bgcolor: '#ffebee', width: 28, height: 28 }}>
                    ...
                    <TextField
                        ...
                        label="Nama Toko"
                        value={namaToko}
                        onChange={(e) => setNamaToko(e.target.value.toUpperCase())}
                        placeholder="Masukkan nama toko..."
                        InputProps={{
                            startAdornment: <StoreIcon sx={{ color: 'text.secondary', mr: 1, fontSize: 20 }} />
                        }}
                        sx={{
                            '& .MuiOutlinedInput-root': {
                                borderRadius: 3,
            ...
            <Paper elevation={0} sx={{
                ...
                <Button
                    ...
                    fullWidth
                    sx={{
                        mb: 2, py: 2, borderRadius: 3, textTransform: 'none',
                        borderStyle: 'dashed', borderWidth: 2,
                        borderColor: foto ? 'primary.main' : '#e0e0e0',
                        backgroundColor: foto ? '#fff5f5' : 'white',
                        transition: 'all 0.3s'
                    }}
                >
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                        <UploadIcon sx={{ fontSize: 32, color: foto ? 'primary.main' : 'text.secondary' }} />
                        <Typography variant="body2" fontWeight="bold" color={foto ? 'primary.main' : 'text.secondary'}>
                            {foto ? 'Foto Terpilih - Klik untuk Ganti' : 'Upload Foto (Wajib)'}
                        </Typography>
                    ...
                    <Fade in timeout={500}>
                        <Box
                            ...
                            sx={{
                                ...
                                borderRadius: 3, mb: 2, border: '2px solid #f5f5f5',
                ...
                <Stack spacing={2} sx={{ mb: 2 }}>
                    ...
                    <Autocomplete
                        ...
                        renderOption={(props, option) => (
                            <Box component="li" {...props} sx={{ py: 1 }}>
                                <Typography sx={{ fontSize: '0.9rem', fontWeight: 500 }}>{option.label}</Typography>
                            </Box>
                        )}
                    ...
                    <TextField
                        ...
                        placeholder="0"
                        InputProps={{
                            startAdornment: <Typography sx={{ mr: 1, color: 'text.secondary', fontWeight: 'bold' }}>Rp</Typography>
                        }}
                        sx={{
                            '& .MuiOutlinedInput-root': {
                                borderRadius: 3,
                        ...
                        helperText="Format otomatis: Rp 1.000"
                    />
                                                ...
                                                <TableCell sx={{ fontWeight: 500, fontSize: '0.8rem', py: 1.5 }}>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                        <Chip
                                                            label={index + 1}
                                                            size="small"
                                                            sx={{ height: 20, fontSize: '0.65rem', bgcolor: '#ffebee', color: 'primary.main' }}
                                                        />
                                                        {row.sku}
                                                    </Box>
                                                </TableCell>
                                                <TableCell align="right" sx={{ fontFamily: 'monospace', fontWeight: 500, fontSize: '0.8rem', py: 1.5 }}>
                                                ...
                                                <TableCell align="center" sx={{ py: 1.5 }}>
                                                    <Tooltip title="Hapus">
                                                        <IconButton
                                                            color="error"
                                                            size="small"
                                                            onClick={() => handleOpenDeleteDialog(row.id, row.sku)}
                                                            sx={{
                                                                transition: 'all 0.2s',
                                                                '&:hover': { transform: 'scale(1.1)', backgroundColor: '#ffebee' }
                                                            }}
                                                        >
            ...
            <Button
                ...
                sx={{
                    py: 2, borderRadius: 3, fontSize: '1rem', fontWeight: 'bold',
                    boxShadow: '0 4px 16px rgba(227,30,36,0.3)',
                    textTransform: 'none',
                    background: isSubmitting ? '#ccc' : 'linear-gradient(135deg, #E31E24 0%, #B71C1C 100%)',
                    transition: 'all 0.3s'
                }}
            >
                {isSubmitting ? (
            ...
            <Dialog
                ...
                onClose={() => setDialogDelete({ open: false, itemId: null, sku: '' })}
                sx={{ '& .MuiDialog-paper': { borderRadius: 4, minWidth: 300 } }}
            >
                <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'error.main', fontSize: '1.05rem' }}>
                    <WarningIcon /> Konfirmasi Hapus
                </DialogTitle>
                <DialogContent>
                    <DialogContentText sx={{ fontSize: '0.9rem' }}>
                        Hapus item <strong>{dialogDelete.sku}</strong> dari queue?
                    </DialogContentText>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    <Button
                        onClick={() => setDialogDelete({ open: false, itemId: null, sku: '' })}
            ...
            <Dialog
                open={dialogSubmit.open}
                onClose={() => setDialogSubmit({ open: false })}
                sx={{ '& .MuiDialog-paper': { borderRadius: 4, minWidth: 320 } }}
            >
                <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'primary.main', fontSize: '1.05rem' }}>
                    <CheckIcon /> Konfirmasi Submit
                </DialogTitle>
                <DialogContent>
                        ...
                        <Stack spacing={1}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                ...
                                <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                                    <strong>Tanggal:</strong> {formatDateLocal(date)}
                                </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <PersonIcon sx={{ fontSize: 18, color: 'primary.main' }} />
                                <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                                    <strong>SPG:</strong> {nama?.label || nama}
                                </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <StoreIcon sx={{ fontSize: 18, color: 'primary.main' }} />
                                <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                                    <strong>Toko:</strong> {namaToko.toUpperCase()}
                                </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <ImageIcon sx={{ fontSize: 18, color: 'primary.main' }} />
                                <Typography variant="body2" sx={{ fontSize: '0.85rem' }}>
                                    <strong>Foto:</strong> {foto ? 'Terupload' : 'Belum ada'}
                                </Typography>
                            </Box>
                            <Divider sx={{ my: 0.5 }} />
                            <Box sx={{
                                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                pt: 1, borderTop: '2px dashed rgba(227, 30, 36, 0.2)'
                            }}>
                                <Typography variant="body2" fontWeight="bold" sx={{ fontSize: '0.85rem' }}>
                                    Total ({queue.length} item)
                                </Typography>
                                <Typography variant="body1" fontWeight="bold" color="primary" sx={{ fontSize: '1rem', fontFamily: 'monospace' }}>
                                    Rp {formatRupiah(grandTotal)}
                                </Typography>
                            </Box>
                        </Stack>
                ...
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    <Button
                        onClick={() => setDialogSubmit({ open: false })}
                        variant="outlined" color="inherit"
                        sx={{ borderRadius: 2, textTransform: 'none', px: 3 }}
                    >
                        Batal
